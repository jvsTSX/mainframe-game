...
; Delay note handle
  ...
  jrs nz, DELAY_End
	...
	ld [CurrntNote], a
	ld a, b
  carl INSTRUMENT_PROCESS ; A is the instrument you want to process
	ld hl, #WorkFlags     ; check pitch reset status
	bit [hl], #00001000b
  jrs nz, DELAY_NoReset
	xor a, a
	ld b, a
	ld [SweepInterp], ba
	ld [OffsetNote], a
	...
	ld [PitchState], a
DELAY_NoReset:
	and [hl], #11110101b  ; disable delay
DELAY_End:
...
INSTRUMENT_PROCESS:
  ...
  jrs z, INSTRPROC_GmacroSkip
	...
	ld ix, iy ; IY for music data access ONLY
	ld iy, [GmacroLocal]
	ld b, #0
	add ba, ba
	add iy, ba
...
; CHANGE THIS JSADKJABSJHKDBFJHK
; use the unused bit in workflags to signal when a delayed event is happening
; that way anything that writes to the duration is discarded
; also a note command and wait command should disable delay to avoid a trap loop (? maybe not i'm just a dingus pokefur why do i do this qwq)

; new delay handler will just take a 16-bit position instead of note+ins and pass the execution out to a command handler similar to the main one
; this adds one more level to stack usage

; new delay command specs:
; 3- XX YY
; X = wait count
; Y = event size (in bytes) + 1

; event size is merely used to skip the delay command and its data
; instead of writing every individual event into RAM, it instead writes down the address ahead of the delay command
; and uses it as soon as the delay counter expires

; it uses the same command fetch code as the main, but in a condition where it will already leave if it reads another
; note command or wait command

; delayed event format
; [ -T NN II ] [cmd] [cmd] [cmd] [cmd] ...
; T = Type (0 = note + ins, 1 = note only, 2 = legato, 3 = FX only/skip)
; N = Note index
; I = Instrument
;
; if NN is '0FFh', then it will treat as no note, just effects
;
; it will stop as soon as it encounters either of these:
;  - play note   (0)
;  - wait        (2)
;  - legato note (E)

; with that in mind you should place your delay command as the last command, if not possible 
; (well it wouldn't matter really, it's all the same frame) just include the first byte of the commands listed above

; example:
; 30h, 05h, 00h, 10h, 1Ah, 02h, 50h, 00h
; |       | |                          |
; +-------+ +--------------------------+
;    cmd            event
;
...
;
;    1 - command stage
; in the command stage it simply just writes the contents of IY (address) + 2, into the event address variables (low and high)
; after it wrote the tick wait count into the delay counter, of course, then it takes the size parameter and offsets IY (address)
; with it and exits the command
;
;    2 - handler stage
; as soon as the delay counter expires the delay handler will execute, it will get the address and read the first byte (type)
; if type is 0, the initial event is 3 bytes, if type is 1 or 2 the initial event is 2 bytes, if type is 3 then there is no initial event
; (1 byte only, for the type alone) then it will push a return address into the stack and call for the main command fetch loop
; with the continue flag initialized to exit as soon as it detects a note or wait command, after it's done it then just disables the
; delay counter and continues executing the rest of block A
;
; to make sure that a 0, 2 or E command's RET won't leave the driver, it will first push a return address before calling the command loop
; because the loop is the very same used by the regular note fetch
...
CMD_PortaCheckBaseNote:
	bit a, #10000000b
  jrs z, CMD_PortaLastNote
...
SFXpos:				ds 2
SFXduty:			ds 1
_ADPM_SFXstatus:    ds 1 ; similar to overlay but it tells you what SFX it's playing
